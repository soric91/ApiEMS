...
async def report_daily(
    ...
    return ApiResponse(data=await build_report(repo, settings, "daily", device_id))
...
async def report_weekly(
    ...
    return ApiResponse(data=await build_report(repo, settings, "weekly", device_id))
...
async def report_monthly(
    ...
    return ApiResponse(data=await build_report(repo, settings, "monthly", device_id))
...
async def report_yearly(
    ...
    return ApiResponse(data=await build_report(repo, settings, "yearly", device_id))
...
async def report_custom(
    ...
    return ApiResponse(data=await build_report(repo, settings, "custom", device_id, from_, to))
